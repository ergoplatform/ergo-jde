...
This model only protects against a disappearing bank and makes following assumptions:
1. The on-chain root digest of user balances published by the bank is correct. 
2. The customers know the entire off-chain tree and can verify the correctness of the on-chain digest at any time.
3. The quantity of bank tokens in bank box exceed the sum of all user balances by at least 1.  
...
Tokens:
1. Bank NFT to identify the box.
