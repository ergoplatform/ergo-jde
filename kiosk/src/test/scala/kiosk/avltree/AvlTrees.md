...
In our context, the AVL tree is used like a Merkle tree: a root digest authenticates the entire data structure. 
...
with Applications to Cryptocurrencies](https://eprint.iacr.org/2016/994.pdf)*), we describe how AVL tres can be used in smart contract.
...
5. Given an AVL tree, we can do following operations:
    ...
    1. Lookup a key and return the corresponding value (inside an `Option`) if it exists or `None` if  it does not. 
...
For instance, for an *Insert* operation to be valid, the following is needed:
...
4. A proof that the root digest after inserting (key, value) is correctly derived from the root digest before insertion.
   The proof implicitly also proves that the key didn't exist in the tree earlier.
       
Similarly, a proof of lookup needs:
1. The root digest.
2. The key to look for.
3. A proof, which either proves that the key exists or does not. Both proofs have an identical structure.
...
Since the proofs of AVL trees are very compact, we can perform operations on very large trees via very small transactions which only contain proofs, root digest(s), and modified keys/values.
...
We describe AVL Tree usage via a simple examples here. 

ErgoScript encodes an AVL tree in an `AvlTree` type, which is essentially the root digest along with parameters such as key and value lengths.

### Insert 

This demonstrates how to perform insert operation in ErgoScript. 
...
This box was [spent on the main-net](https://explorer.ergoplatform.com/en/addresses/2ma4cozJw8VuB2GmYXCeHr72R9w9S9kHynUJUrMV8u7DDuh5EVJ2iWmRagfh4CqeCGRch3p9Tvmq16Fr9xNsuD7geFMJur) using proofs generated via AppKit as described [here](ops/AvlInsertSpec.scala).

### Remove

This demonstrates how to perform remove operation in ErgoScript.
...
This box was [spent on the main-net](https://explorer.ergoplatform.com/en/transactions/795c32aaade3ff4a0814db9e7d69b28cb2714c2796a68250a9cdf73b75dc4957) using proofs generated via AppKit as described [here](ops/AvlRemoveSpec.scala).

### Lookup (Exists)

This demonstrates how to perform lookup operation in ErgoScript where the key exists.
...
This box was [spent on the main-net](https://explorer.ergoplatform.com/en/transactions/083fd0e35ea4706e53f80c29708f3c5337992ac7b4188f0dbcdd3d9597d54d45) using proofs generated via AppKit as described [here](ops/AvlLookupSpec.scala).

### Lookup (Does Not Exist)

This demonstrates how to perform lookup operation in ErgoScript where the key does not exist.
...
This box was [spent on the main-net](https://explorer.ergoplatform.com/en/transactions/c9c7f16a754065b3d2478a693d96ead241eada03d3222751c703befca305a4f2) using proofs generated via AppKit as described [here](ops/AvlNotExistSpec.scala).
