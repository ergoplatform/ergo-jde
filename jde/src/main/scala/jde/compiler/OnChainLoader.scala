...
import jde.compiler.model.Program
...
class OnChainLoader(explorer: Explorer)(implicit dictionary: Dictionary) {
  ...
  def load(program: Program) = {
    optSeq(program.auxInputUuids).zipWithIndex.foreach { // fetch aux-input boxes from explorer and load into dictionary
    ...
    optSeq(program.dataInputUuids).zipWithIndex.foreach { // fetch data-input boxes from explorer and load into dictionary
    ...
    optSeq(program.inputUuids).zipWithIndex.foreach { // fetch input boxes from explorer and load into dictionary
